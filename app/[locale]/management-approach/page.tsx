...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const page = await getPageByTemplate("template-management-approach", typedLocale);
  const pathname = getPathname({ locale: typedLocale, href: "/management-approach" });
...
export default async function ManagementApproachPage({ params }: PageProps) {
  const { locale } = await params;
  const typedLocale = locale as Locale;
  const page = await getPageByTemplate("template-management-approach", typedLocale);
